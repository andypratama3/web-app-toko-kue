<aside
    ...
    {{-- Bagian Logo (flex-shrink-0 agar tidak menyusut) --}}
    <div class="flex-shrink-0 h-19">
        <i class="absolute top-0 right-0 p-4 opacity-50 cursor-pointer fas fa-times dark:text-white text-slate-400 xl:hidden"
            sidenav-close></i>
        <a id="sidebar-logo-link" class="flex items-center block px-8 py-6 m-0 text-base whitespace-nowrap dark:text-white text-slate-700"
            href="{{ $dashboardUrl }}">
            <img src="{{ asset('assets/homepage/logo.png') }}"
                class="w-auto h-8 transition-all duration-200 ease-nav-brand" alt="main_logo" />
            <span id="sidebar-logo-text" class="ml-2 text-xl font-semibold transition-all duration-200 ease-nav-brand text-greenlight sidenav-text">
                Kue Pandan Asli
            </span>
        </a>
    </div>
    ...
    {{-- Area menu yang bisa di-scroll (flex-grow agar mengisi ruang) --}}
    <div class="flex-grow w-full overflow-y-auto">
        <ul class="flex flex-col pl-0 mb-0">
            ...
            <li class="w-full mt-4 mb-4">
                <h6 class="pl-6 ml-2 text-xs font-bold leading-tight uppercase sidenav-text dark:text-white opacity-60">Performa
                    Section
                </h6>
            </li>
            ...
            <li class="w-full mt-4 mb-4">
                <h6 class="pl-6 ml-2 text-xs font-bold leading-tight uppercase sidenav-text dark:text-white opacity-60">Account Settings
                </h6>
            </li>
            <li class="mt-0.5 w-full">
            ...
            <li class="mt-0.5 w-full">
                <a class="py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors rounded-lg font-semibold @if (request()->routeIs('kurir.profile')) bg-blue-500/13 text-blue-700 dark:text-white dark:opacity-80 @else dark:text-white dark:opacity-80 @endif"
                    ...
                    <div
                        ...
                        <i
                            class="ni ni-single-02 @if (request()->routeIs('kurir.profile')) text-blue-500 @else text-slate-700 @endif"></i>
                    </div>
    ...
    {{-- Tombol toggle (flex-shrink-0 agar tidak menyusut) --}}
    <div class="flex-shrink-0 p-4 mx-4">
        <button id="sidebar-toggle-button" class="hidden xl:flex items-center justify-center w-full p-3 text-white bg-greenlight rounded-full hover:bg-black dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-black focus:outline-none">
