...
@push('page-scripts')
    <script>
        ...
        // --- PERUBAHAN 1: Buat map status di JavaScript dari data PHP ---
        ...
        // ... (Semua fungsi async fetchOrderDetails, populateOrderDetailsModal, dll, tetap sama) ...
        async function fetchOrderDetails(orderId) {
            openModal('orderDetailsModal');
            const modalLoader = document.getElementById('modalLoader');
            const modalContent = document.getElementById('modalContent');
        ...
        //     // --- PERUBAHAN 2: Gunakan map yang sudah dibuat untuk mendapatkan teks yang benar ---
        ...
        function updateTableRowStatus(orderId, newStatus) {
            ...
            // Definisikan kelas warna dan bar
            let newColorClasses = 'bg-gray-100 text-gray-800';
            let newColorBarClass = 'bg-gray-400';

            switch (newStatus) {
                case 'diambil':
                    newColorClasses = 'bg-blue-100 text-blue-800';
                    newColorBarClass = 'bg-blue-500';
                    break;
                case 'diantar':
                    newColorClasses = 'bg-yellow-100 text-yellow-800';
                    newColorBarClass = 'bg-yellow-500';
                    break;
                case 'diterima_pembeli':
                    newColorClasses = 'bg-purple-100 text-purple-800';
                    newColorBarClass = 'bg-purple-500';
                    break;
                case 'menunggu_retur':
                    newColorClasses = 'bg-red-100 text-red-800';
                    newColorBarClass = 'bg-red-500';
                    break;
                case 'menunggu_verifikasi_admin':
                    newColorClasses = 'bg-orange-100 text-orange-800';
                    newColorBarClass = 'bg-orange-500';
                    break;
                case 'selesai':
                    newColorClasses = 'bg-green-100 text-green-800';
                    newColorBarClass = 'bg-green-500';
                    break;
            }
            ...
            rows.forEach(row => {
                ...
                if (statusSpan) {
                    ...
                    // 1. Ambil kelas dasar yang mendefinisikan bentuk dan ukuran
                    const baseClasses = 'status-badge flex-shrink-0 px-2.5 py-1 text-xs font-semibold rounded-full whitespace-nowrap';

                    // 2. Gabungkan kelas dasar dengan kelas warna yang baru
                    statusSpan.className = `${baseClasses} ${newColorClasses}`;
                }

                const colorBar = row.querySelector('.absolute.top-0.left-0');
                if (colorBar) {
                    // Hapus semua kelas bg-* lama dan tambahkan yang baru
                    colorBar.className = colorBar.className.replace(/bg-\w+-\d+/g, '');
                    colorBar.classList.add(newColorBarClass);
                }
            });
        }
        
        // ... (sisa fungsi JavaScript Anda tetap sama) ...
        function openReturnProductModal(order) {
            const returnModalLoader = document.getElementById('returnModalLoader');
            const returnModalContent = document.getElementById('returnModalContent');
            const returnProductList = document.getElementById('returnProductList');
