            ...
            try {
                ...
                if (Array.isArray(data.items) && data.items.length > 0) {
                    data.items.forEach(item => {
                        ...
                        priceLineEl.innerHTML = `${formatRupiah(price)} &rarr; ${formatRupiah(finalSubtotal)}`;

                        elements.productDetails.appendChild(clone);
                    });
                } else {
                     elements.productDetails.innerHTML = '<p class="text-center text-gray-500">Tidak ada produk.</p>';
                }
